...
class DonorFeedbackController extends GetxController {
  final DonateAcceptModel payload;
  final Completer<GoogleMapController> mapController;
  DonorFeedbackController({required this.payload, required this.mapController});
  ...
  Future<void> getRating() async {
    takerRating = 0.0;
    takerRating = await getDonorRating(payload.donorEmail);
    if (takerRating == null) {
      takerRating = 0.0;
    }
    update();
  ...
  Future<double?> getDonorRating(String email) async {
    try {
      return await _homeRepository.getDonorRating(email);
    } catch (e) {
      Helper.handleError(e, 'Error while geting rating!');
      return null;
    }
  }
  ...
  Future<bool> updateAcceptDonationData(Map<String, dynamic> payload) async {
    try {
      showLoader('updating...');
      return await _homeRepository.updateAcceptDonationData(payload);
    } catch (e) {
      Helper.handleError(e, 'Error while updating status!');
      return false;
    } finally {
